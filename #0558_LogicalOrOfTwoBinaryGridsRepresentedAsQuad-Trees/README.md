...
## Solution
It's not much harder than [427. Construct Quad Tree](https://github.com/BlueBug12/LeetCode-Solution/tree/master/%230427_ConstructQuadTree). Benefit from the **logical bitwise OR**, we can construct the tree much faster. There are two tips that can save our time to traverse two quad-trees:
1. If the `quadtree` node is a leaf whose value is `1`, then
add this node to the new quadtree. Because `a | 1 = 1`, no matter what the other quadtree node is, the result node must be a `1` leaf.
2. If the `quadtree` node is a leaf whose value is `0`, then
add the node of the other quadtree to the new quadtree. Because `a | 0 = a`, the result node always equal to the other one.

Moreover, if the mentioned two tips can't apply to the current node, which means their are both `non-leaf` nodes. W e just need to traverse two quadtrees deeper until it meets one of the tips.

_**Time complexity: O(min_size(quadTree1,quadTree2))**_

## Summary
Make good use of the **bitwise** operation helps a lot, especially from the view of hardware.
